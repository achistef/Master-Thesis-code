...
def create_plot(data, mean_files, var_files, labels, ylabel, xlabel, title):
    ...
    ax.set_title(title)
    ax.legend(loc=10, bbox_to_anchor=(0.35, 0.2))
    fig.tight_layout()


def create_msae_plots(data, title):
    create_plot(data, mse_mean_files, mse_var_files, labels, 'MSE', 'Number of heads', title)
    create_plot(data, mae_mean_files, mae_var_files, labels, 'MAE', 'Number of heads', title)
