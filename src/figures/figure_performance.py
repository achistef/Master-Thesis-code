...
def create_plot(data, mean_files, var_files, labels, ylabel, xlabel, xvalues, title):
    fig, ax = pl.subplots(figsize=(10, 10))
    for c, algorithm in enumerate(algorithms):
        for mean_file, var_file, label in zip(mean_files, var_files, labels, ):
            for option in options:
                line = data[algorithm][option][mean_file]
                error = data[algorithm][option][var_file]
            ax.plot(xvalues, line, linewidth=5, label=algorithm + ' ' + label, marker=markers[c], markersize=20)
...
def create_msae_plots(data, title, xvalues):
    create_plot(data, mse_mean_files, mse_var_files, labels, 'MSE', '$t_{train}$', xvalues, title)
    create_plot(data, mae_mean_files, mae_var_files, labels, 'MAE', '$t_{train}$', xvalues, title)


def plot_everything_from(path, title, xlabel):
    data = load_data_from(path)
    create_msae_plots(data, title, xlabel)


plot_everything_from(big_dataset_path, 'Live video streaming event', list(range(20, 30)))
plot_everything_from(small_dataset_path, 'Live test', list(range(10, 20)))
pl.show()
